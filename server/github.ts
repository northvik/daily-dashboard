 ...
 */
export async function fetchPRsUpdatedSince(sinceDate: string): Promise<GHSearchItem[]> {
...
export async function fetchReviewsBatch(
  ...
  const data = await ghGraphQL<Record<string, { pullRequest?: {
    reviewDecision: string | null;
    latestReviews?: { nodes: { state: string; author: { login: string } }[] };
  } }>>(`{ ${fragments.join("\n")} }`);
...
export async function fetchGitSpiceStacks(
  ...
  const data = await ghGraphQL<Record<string, {
  ...
  for (let i = 0; i < prs.length; i++) {
    ...
    const nums = [...gsComment.body.matchAll(/#(\d+)/g)].map((m) => Number(m[1]));
    if (nums.length > 0) {
