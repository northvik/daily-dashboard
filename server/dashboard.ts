...
export async function fetchDashboard() {
  ...
  const headRefs = new Map(
    detailResults.filter((r) => r.detail).map((r) => [r.key, r.detail!.head.ref]),
  );
  ...
  const allPRs: PR[] = ghItems.map((item) => {
    ...
    const baseRef = extractBaseRef(item.body ?? "") ?? detail?.base.ref ?? "main";
    ...
    return {
      ...
      base: baseRef,
      status: detail ? deriveStatus(detail, item.draft, reviewInfo) : item.draft ? "draft" : "review",
      depth: 0,
    };
  });
  ...
  const mergedToFetch = new Map<string, { owner: string; repo: string; num: number }>();
  ...
  for (const [key, stackNums] of gsStacks) {
    ...
    for (let i = 1; i < stackNums.length; i++) {
      const pr = allPRs.find((p) => p.repo === repo && p.number === stackNums[i]);
  ...
  try {
    staticOverrides = JSON.parse(
      readFileSync(resolve(process.cwd(), "data/stack-overrides.json"), "utf-8"),
    );
  ...
  const liveOverrides = await enrichSummaries(summaryInput).catch(() => [] as SummaryOverride[]);
