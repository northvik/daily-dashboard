...
export function extractTicket(
  ...
  const urlMatch = body?.match(
    new RegExp(`https://linear\\.app/[^\\s)]*${ticket.replace("-", "[-/]")}[^\\s)]*`),
  );
  return {
    ticket,
...
export function extractFirstSentence(body: string): string {
  for (const line of body.split("\n")) {
    const t = line.trim();
    if (!t || t.startsWith("#") || t.startsWith("<!--") || t.startsWith(">") || t.startsWith("**Ticket")) {
      continue;
    }
    const clean = t.replace(/\[([^\]]+)\]\([^)]+\)/g, "$1");
    if (clean.length > 10) return clean.slice(0, 120) + (clean.length > 120 ? "…" : "");
  }
  return "";
}
...
export function deriveStatus(
  ...
  if (reviewInfo) {
    if (reviewInfo.latestReviews.some((r) => r.state === "CHANGES_REQUESTED")) return "changes-requested";
    if (reviewInfo.reviewDecision === "APPROVED") return "approved";
    if (reviewInfo.latestReviews.some((r) => r.state === "COMMENTED")) return "commented";
  }

  if (detail.mergeable_state === "behind") return "rebase";
  if (detail.mergeable_state === "unstable" || detail.mergeable_state === "blocked") return "ci-fail";
...
export function buildGroups(
  ...
  for (const pr of prs) {
    ...
    if (numMatch) {
      ...
      if (byKey.has(parentKey)) { childOf.set(key, parentKey); continue; }
    }
    if (!TRUNK_BRANCHES.has(pr.base)) {
      const parentKey = branchToKey.get(pr.base);
  ...
  for (const [, members] of grouped) {
    ...
    if (ticket) {
      ...
      for (const k of members) { ticketGroups.get(ticket)!.add(k); assigned.add(k); }
