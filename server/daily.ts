...
export function loadDaily(date: string): DailyData | null {
  try {
    const raw = JSON.parse(readFileSync(dataPath(date), "utf-8")) as DailyData & {
      yesterday?: unknown;
      today?: unknown;
    };
    return {
...
async function gatherRawContext(date: string): Promise<RawContext> {
  ...
  const [updated, merged, open, linearRecent, linearStarted] = await Promise.all([
...
function dedupeBullets(title: string, bullets: DailyBullet[]): DailyBullet[] {
  ...
  for (const b of bullets) {
    ...
    if (titleL.includes(text.toLowerCase()) && text.length < title.length + 5) continue;
  ...
  for (const item of raw) {
    ...
    if (Array.isArray(o.bullets)) {
      ...
      if (looksLikePrRef(title) && title.length < 40) {
        title = cleanTitle(typeof o.title === "string" ? o.title : "") || "Misc work";
      }
      ...
      const bullets = dedupeBullets(
        ...
        o.bullets
          .map((b): DailyBullet | null => {
            ...
            const tone =
              bb.tone === "blocked" || bb.tone === "done" || bb.tone === "default"
                ? bb.tone
                : undefined;
      ...
      if (Array.isArray(o.tags)) {
        for (const t of o.tags) {
          if (typeof t === "string") {
            ...
            if (typeof tt.label === "string" && !tags.some((x) => x.label === tt.label)) {
              tags.push({
                label: tt.label,
...
async function enrichWithCursor(
  ...
  const slackResearch = slackId
    ? [
        "1. Slack: slack_search_public_and_private / slack_search_public:",
        `   from:<@${slackId}> after:${ctx.yesterdayDate} before:${ctx.todayDate}`,
        "   Infer commitments, blockers, decisions.",
        "2. Use Linear/GitHub links below for more context if helpful.",
      ]
    : [
        "1. No Slack user id configured — skip Slack MCP.",
        "2. Use Linear/GitHub links below for more context if helpful.",
      ];
  ...
  const prompt = [
    ...
    "- alerts: only real blockers / missing tickets — max 2, or empty array",
    '- alert type: "missing-ticket" | "stale-pr" | "slack-gap"',
    "",
  ...
  try {
    ...
    if (result.status !== "finished" || !result.result) {
      console.error("[daily] Cursor status:", result.status);
      return null;
    }
...
export async function generateDaily(
  ...
  console.log(`[daily] Generating for ${date} (run #${generationCount}${opts.force ? ", forced" : ""})…`);
