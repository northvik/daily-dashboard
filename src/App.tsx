...
function IconChangesRequested() {
  return (
    <svg
      viewBox="0 0 16 16"
      fill="currentColor"
      className="status-icon changes-requested"
    >
...
function IconCommented() {
  return (
    <svg
      viewBox="0 0 16 16"
      fill="currentColor"
      className="status-icon commented"
    >
  ...
  return (
    <div className="pr-row">
      ...
      <a
        ...
        target="_blank"
        rel="noopener"
        className={`pr-title-link${pr.merged ? ' merged' : ''}`}
      >
        {pr.title}
      </a>
      ...
      <details open>
        ...
        <ul>
          {node.children.map((child) => (
            <TreeNodeView
              key={`${child.pr.repo}#${child.pr.number}`}
              node={child}
            />
          ))}
        </ul>
    ...
    <div className="stack-card">
      <div className="stack-header">
        ...
        <div className="stack-header-right">
          {ticket?.labels?.map((l) => (
            <span key={l} className="ticket-label">
              {l}
            </span>
          ))}
          {ticket && (
      ...
      {tree.length > 0 && (
        <div className="stack-body">
          <ul className="tree">
            {tree.map((node) => (
              <TreeNodeView
                key={`${node.pr.repo}#${node.pr.number}`}
                node={node}
              />
            ))}
          </ul>
...
function Legend() {
  return (
    <div className="legend">
      ...
      <span className="legend-item">
        <IconRebase /> Needs rebase
      </span>
      <span className="legend-item">
        <IconCross /> CI failing
      </span>
      <span className="legend-item">
        <IconChangesRequested /> Changes requested
      </span>
      <span className="legend-item">
        <IconCommented /> Commented
      </span>
      <span className="legend-item">
        <IconEye /> Awaiting review
      </span>
      <span className="legend-item">
        <IconDraft /> Draft
      </span>
...
function DailyPanel() {
  ...
  const generatedLabel = daily
    ? new Date(daily.generatedAt).toLocaleTimeString([], {
        hour: '2-digit',
        minute: '2-digit',
      })
    : null
...
export default function App() {
  ...
  const openPRs =
    data?.groups.flatMap((g) => g.prs).filter((p) => !p.merged) ?? []
  ...
  return (
    <>
      ...
      <div className="stats-row">
        ...
        {readyCount > 0 && (
          <span className="stat green">✓ {readyCount} Ready</span>
        )}
        {rebaseCount > 0 && (
          <span className="stat orange">↑ {rebaseCount} Need rebase</span>
        )}
        {ciFailCount > 0 && (
      ...
      {!data && loading && (
        <div className="loading">Fetching PRs and tickets…</div>
      )}
